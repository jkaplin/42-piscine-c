...
/*   Updated: 2018/11/11 23:40:23 by jkaplin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < NUM_RUSHES)
	{
		if (str_match(input_str, f_ptr[i].rush, input_width, input_height))
		{
			...
			if (i < (NUM_RUSHES - 1) && (str_match(input_str, f_ptr[i + 1].rush,
				input_width, input_height) || str_match(input_str, f_ptr[i + 2].rush,
				input_width, input_height) || str_match(input_str, f_ptr[i + 3].rush,
